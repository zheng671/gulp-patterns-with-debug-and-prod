...
/**
 * Create web api url constant
 * @return {Stream}
 */
gulp.task('constantDev', function () {
    log('Creating an AngularJS constant');
    var myConfig = require('./web.debug.js')();
    return $.ngConstant({
        'name': 'app',
        'constants': myConfig,
        'stream': true,
        'space': '    ',
        'templatePath': config.src + 'tpls/app.constant.tpl.ejs'
    })
    .pipe($.rename('app.constant.js'))
    .pipe(gulp.dest(config.app));
});

gulp.task('constantProd', function () {
    log('Creating an AngularJS constant');
    var myconfig = {};
    if (args.prod) {
        myconfig = require('./web.prod.js')();
    } else {
        myconfig = require('./web.prerelease.js')();
    }
    log('Creating an AngularJS constant');
    return $.ngConstant({
        'name': 'app',
 ...
 */
gulp.task('build', ['constantProd', 'optimize', 'images', 'fonts'], function () {
...
gulp.task('optimize', ['inject'], function () {
    ...
    return gulp
        .src(config.index)
        .pipe($.plumber())
        .pipe($.inject(gulp.src(templateCache), { starttag: '<!-- inject:template:js -->' }))
 ...
 * --nosync
 */
gulp.task('serve-dev', ['constantDev', 'inject'], function () {
    serve(true /*isDev*/);
});
